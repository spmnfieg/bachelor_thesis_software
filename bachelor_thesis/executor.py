...
def valid(changes):
    return changes is not None and len(changes) > 1 and (
            len(changes[changes['Buggy'] > 0]) > 5 and len(changes[changes['Buggy'] == 0]) > 5)
...
def performPredictionRunner(projectName, originalTrainChanges, originalTestChanges, trainReleaseDate, testReleaseDate,
                            trainApproach, returnResults=False):
    ...
    for fselector in ['CFS']:
        ...
        trainChangesSMOTE, tuneChangesProcessed, testChangesProcessed = getFreshCopy(
            originalTrainChanges.copy(deep=True), originalTestChanges.copy(deep=True), trainApproach)
        ...
        for learner in THE_LEARNERS_LIST:
            ...
            if Dummy_Flag == True:
                trainChangesCpy, tuneChangesCpy, testChangesCpy = trainChanges.copy(deep=True), tuneChanges.copy(
                    deep=True), testChanges.copy(deep=True)
            else:
            ...
            if Dummy_Flag and getTrue(trainY) > 0 and getTrue(tuneY) > 0 and getFalse(trainY) > 0 and getFalse(
                    tuneY) > 0:
            ...
            if UNIT_TEST == False:
                ...
                print("***************** ", classifierName, fselector, trainChangesCpy.columns.tolist(),
                      testChangesCpy.columns.tolist(), " Results **************************")
                if metricsReport is not None:
